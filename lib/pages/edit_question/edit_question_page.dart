...
class _EditQuestionPageState extends State<EditQuestionPage> with TickerProviderStateMixin {
  ...
  bool _previewShrunk = false;
  double _floatingHeight = -1;
  ...
  Future<void> _batchSetJump() async {
    ...
    if (result == null || result == kJumpLogicDialogCancelled) return;
    setState(() {
      final clearJump = result == kJumpLogicDialogDefaultNext;
      final newDest = clearJump ? null : result;
      for (int i = 0; i < _options.length; i++) {
        final opt = _options[i];
        _options[i] = opt.copyWith(
          destination: newDest,
          clearDestination: clearJump,
        );
        if (clearJump) {
            ...
            children: [
              ...
              FHeader.nested(
                ...
                suffixes: [
                  if (!isDesktop)
                    FHeaderAction(
                      icon: Icon(_previewShrunk ? FIcons.shrink : FIcons.expand, size: 20),
                      onPress: () {
                        setState(() {
                          _previewShrunk = !_previewShrunk;
                        });
                      },
                    ),
                ],
  ...
  Widget _buildMobileLayout(ThemeData theme) {
    return LayoutBuilder(
      builder: (context, constraints) {
        final defaultPanelHeight = constraints.maxHeight * 0.5;
        final panelHeight = _floatingHeight < 0
            ? defaultPanelHeight
            : _floatingHeight.clamp(120.0, constraints.maxHeight * 0.85);
        final displayHeight = _isDragging ? _dragHeight : panelHeight;
        final panelTweenEnd = _previewShrunk ? 1.0 : 0.0;
        return Stack(
          children: [
            TweenAnimationBuilder<double>(
              tween: Tween(begin: 0, end: panelTweenEnd),
              duration: const Duration(milliseconds: 280),
              curve: Curves.easeOutCubic,
              builder: (context, t, child) {
                return Padding(
                  padding: EdgeInsets.fromLTRB(16, 16 + t * panelHeight, 16, 16),
                  child: child,
                );
              },
              child: Form(
            ...
            AnimatedPositioned(
              ...
              left: 0,
              right: 0,
              height: _previewShrunk ? displayHeight : 0,
              child: IgnorePointer(
                ignoring: !_previewShrunk,
                child: _buildFloatingPreviewPanel(theme, displayHeight, constraints.maxHeight),
              ),
            ),
          ],
        );
  ...
  Widget _buildFloatingPreviewPanel(ThemeData theme, double panelHeight, double maxHeight) {
    return Material(
        ...
        children: [
          Padding(
            ...
            child: SingleChildScrollView(
              child: _buildTitlePreview(theme),
            ),
          ),
          Positioned(
            left: 0,
            ...
            child: GestureDetector(
              ...
              onVerticalDragStart: (_) {
                _dragHeight = _floatingHeight < 0 ? panelHeight : _floatingHeight;
                _isDragging = true;
              },
              onVerticalDragUpdate: (details) {
                setState(() {
                  _dragHeight = (_dragHeight + details.delta.dy).clamp(120.0, maxHeight * 0.85);
                });
              },
              onVerticalDragEnd: (_) {
                setState(() {
                  _floatingHeight = _dragHeight;
                  _isDragging = false;
                });
              },
              child: SizedBox(
    ...
    if (_selectedType == QuestionType.slider) {
      ...
      return OptionsEditor(
        ...
        onSetJumpLogic: (option) async {
          ...
          if (result != null && result != kJumpLogicDialogCancelled) {
            setState(() {
              final clearJump = result == kJumpLogicDialogDefaultNext;
              if (clearJump) {
                _jumpLogic.remove(option.id);
              ...
              if (idx != -1) {
                _options[idx] = _options[idx].copyWith(
                  destination: clearJump ? null : result,
                  clearDestination: clearJump,
                );
              }
            });
