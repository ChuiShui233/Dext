              ...
              Future.microtask(() {
                if (dialogContext.mounted) {
                  ...
                  widget.apiService!.getProjects().then((loadedProjects) {
                    if (dialogContext.mounted) {
                      setDialogState(() {
                        existingProjectNames = loadedProjects
                      ...
                      child: FDialog(
                              ...
                              FTextField(
                                ...
                                description: Text(
                                  nameController.text.trim().isNotEmpty &&
                                          existingProjectNames.contains(nameController.text.trim().toLowerCase())
                                      ? '已存在同名项目'
                                      : '',
                        ...
                        actions: [
                          FButton(
                            ...
                            onPress: isSubmitting ? null : () => closeDialogWithAnimation(null),
                            child: const Text('取消'),
                          ),
                          FButton(
                            onPress: isSubmitting ||
                                    (nameController.text.trim().isNotEmpty &&
                                        existingProjectNames.contains(
                                ...
                                : () async {
                                    ...
                                    try {
                                      ...
                                      final isDuplicate = errorText.contains('已存在同名项目');
                                      showFToast(
                                        context: context,
                                        alignment: FToastAlignment.bottomRight,
                                        title: Text(isDuplicate ? '创建失败' : '创建失败'),
                                        description: Text(isDuplicate ? '已存在同名项目，请使用其他名称' : '创建项目失败: $e'),
                                        suffixBuilder: (context, entry) => IntrinsicHeight(
                                          child: FButton(
                                            style: context.theme.buttonStyles.primary.call,
                                            onPress: entry.dismiss.call,
                            ...
                            child: Text(isSubmitting ? '创建中...' : '创建'),
                          ),
                        ],
                      ),
                    ),
