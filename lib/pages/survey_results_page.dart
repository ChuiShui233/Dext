...
class _SurveyResultsPageState extends State<SurveyResultsPage> with TickerProviderStateMixin {
  ...
  final _exportController = OverlayPortalController();
  ...
  Future<void> _deleteAnswer(int answerId) async {
    try {
      ...
      final result = _results.firstWhere((r) => r.id == answerId);
      ...
      if (mounted) {
        showFToast(
          ...
          description: Text('已删除用户「${result.userAccount}」的作答结果'),
  ...
  Future<void> _batchDeleteAnswers() async {
    ...
    try {
      ...
      if (mounted) {
        showFToast(
          context: context,
          title: const Text('批量删除成功'),
          description: Text('已删除 ${_selectedResults.length} 条作答记录'),
          suffixBuilder: (context, entry) => IntrinsicHeight(
            child: FButton(
              style: context.theme.buttonStyles.primary.call,
              onPress: entry.dismiss.call,
                  ...
                  children: [
                    FPortal(
                               ...
                               child: SizedBox(
                                 ...
                                 child: Column(
                                   ...
                                   children: [
                                     ...
                                     LayoutBuilder(
                                       builder: (context, constraints) {
                                         final narrow = constraints.maxWidth < 240;
                                         final csvBtn = _ExportAction(
                                           label: '导出 CSV',
                                           onTap: () {
                                             controller.hide();
                                             _exportCsv();
                                         ...
                                         return narrow
                                             ? Column(children: [csvBtn, const SizedBox(height: 8), xlsxBtn])
                                             : Row(children: [
                      ...
                      builder: (context, controller, _) => FButton(
                        ...
                        child: const Row(
                          children: [
                            Icon(FIcons.arrowBigUpDash, size: 24),
                            SizedBox(width: 6),
                            Text('导出为...'),
                          ],
                        ),
