...
class ApiCore {
  ...
  Future<http.Response> httpRequest(
    ...
    try {
      ...
      // 处理 401 并重试
      if (response.statusCode == 401 && allowRetry && onTokenRefresh != null) {
        final refreshed = await onTokenRefresh!.call();
        if (refreshed) {
          return httpRequest(method, url, data: data, onStatus: onStatus, allowRetry: false);
        }
